...
const DeleteUserService = async (userId: string) => {
    ...
    try {
        await session.withTransaction(async () => {
            ...
            const isUserFound = await User.findById(userId).session(session).lean().exec();
            if (!isUserFound) {
                throw new Error("User not found");
            }

            // 2b. Get all the IDs of the boards the user owns
            const boards = await Board.find({user_id: userId}).select("_id").session(session).lean().exec();
            ...
            if (boards.length > 0) {
                for (let board of boards) {
                    ...
                    if (!result.success) {
                        throw new Error(`Failed to delete board ${boardId}: ${result.message}`)
                    };
                }
            }

            // 2d. Delete the user account
            const {deletedCount} = await User.deleteOne({ _id: userId }).session(session).lean().exec();
            if (deletedCount === 0) {
