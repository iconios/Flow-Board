...
const TaskReorderUtility = async (
  ...
  try {
    ...
    console.log(
      `Task reorder attempt - User: ${userId}, Task: ${taskId}, Target List: ${listId}, Position: ${position}`,
    );
    // 2. Call the Update Task Service with the parameters
    ...
    const errorMessage =
      error instanceof Error ? `${error.message}` : "Error reordering task";
