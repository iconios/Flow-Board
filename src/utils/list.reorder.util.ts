...
import { ListReorderInputSchema, type ListReorderInputType } from "../types/list.type.js";
import { ZodError } from "zod";


const ListReorderUtility = async (socket: Socket, payload: ListReorderInputType) => {
    // 1. Accept the List reorder parameters - userId, listId, and list newPosition
    ...
    try {
        ...
        console.log(`List reorder attempt - User: ${userId}, List: ${listId}, New Position: ${position}`);
        ...
        const errorMessage = error instanceof Error ? `${error.message}` : "Error reordering list"
