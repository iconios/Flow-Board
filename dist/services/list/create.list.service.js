...
import { CreateListInputSchema, } from "../../types/list.type.js";
import mongoose, { MongooseError } from "mongoose";
import Board from "../../models/board.model.js";
import BoardMember from "../../models/boardMember.model.js";
import { produceActivity } from "../../redis/activity.producer.js";
const CreateListService = async (userId, createListInput) => {
    ...
    try {
        ...
        await session.withTransaction(async () => {
            ...
            const duplicate = await List.findOne({
                ...
                .exec();
            if (duplicate) {
                throw new Error("Same title not allowed for two lists within the same board");
            }
            ...
            // 5. Add the list to the board
            await Board.updateOne({ _id: boardId }, { $addToSet: { lists: createdList?._id } }, { session });
            // 6. Send op status to client
            result = {
                success: true,
        ...
        if (result) {
            ...
            }).catch((err) => console.error(`Activity log failed for list creation: ${listId.toString()}`, err));
            return result;
        }
        else {
...
//# sourceMappingURL=create.list.service.js.map