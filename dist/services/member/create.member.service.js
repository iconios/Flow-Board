...
import { CreateMemberInputSchema, } from "../../types/member.type.js";
...
const CreateMemberService = async (boardOwner, createMemberInput) => {
    try {
        // 1. Accept and validate owner id, board id, user email, and user role (if available)
        ...
        if (!userFound) {
            return {
                success: true,
                message: "Inform the user to check his/her email to accept the board member invite",
            };
        }
        ...
        const [boardWithOwnerExists, boardMemberExists, memberExists, boardOwnerDetails,] = await Promise.all([
        ...
        // 8. Send board invite to the user
        sendMemberInvite(userEmail, userFound.firstname, createdMember.verificationToken, boardOwnerDetails.firstname);
        // 9. Produce activity log
        ...
        }).catch((err) => console.error(`Activity log failed for member creation: ${createdMember._id.toString()}`, err));
        // 10. Send op status to the client
        return {
            success: true,
            message: "Inform the user to check his/her email to accept the board member invite",
        ...
        console.error(`Error creating ${createMemberInput.userEmail} as a member for ${createMemberInput.board_id}`, error);
