...
import { UpdateBoardInputSchema, } from "../../types/board.type.js";
import { ZodError } from "zod";
import { produceActivity } from "../../redis/activity.producer.js";
const UpdateBoardService = async (board_id, boardDetailsInput) => {
    try {
        // 1. Receive and validate the user id
        ...
        // 4. Update the board details on the DB
        const updatedBoard = await Board.findByIdAndUpdate(boardId, validatedInput, {
            new: true,
            runValidators: true,
        }).exec();
        if (!updatedBoard) {
        ...
        void produceActivity({
            ...
            object: "Board",
            objectId: boardId,
        }).catch((err) => console.error(`Activity log failed for board update: ${boardId}`, err));
        // 5. Send the status to the client
        return {
            success: true,
...
//# sourceMappingURL=update.board.service.js.map