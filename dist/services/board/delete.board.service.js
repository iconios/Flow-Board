...
const DeleteBoardService = async (boardId, userId, clientSession) => {
    ...
    const run = async () => {
        ...
        if (lists.length > 0) {
            ...
            // 6. Find all the tasks associated with each list in the board
            const allTasks = await Task.find({ listId: { $in: listIds } }).session(session);
    ...
    try {
        ...
        }).catch((err) => console.error(`Activity log failed for board deletion: ${boardObjId.toString()}`, err));
        return {
            success: true,
