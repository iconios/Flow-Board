...
const UpdateTaskService = async (userId, taskId, updateData) => {
    try {
        ...
        if (listId && listForTask.listId?.toString() !== listId) {
            ...
            try {
                await session.withTransaction(async () => {
                    ...
                    const updatedTask = await Task.findByIdAndUpdate(task, newUpdateData, { new: true, runValidators: true })
                        .session(session)
                        .exec();
                    if (!updatedTask) {
        ...
        void produceActivity({
            ...
            object: "Task",
            objectId: taskId,
        }).catch((err) => console.error(`Activity log failed for task update: ${taskId}`, err));
        // 6. Send the updated task to the user
        return {
            success: true,
            message: "Task updated successfully",
