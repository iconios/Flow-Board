...
const DeleteTaskService = async (userId, taskId) => {
    ...
    try {
        await session.withTransaction(async () => {
            ...
            // 3. Remove the task id from its associated list
            const taskRemovedFromList = await List.findOneAndUpdate({ tasks: task }, { $pull: { tasks: task } })
                .select("_id")
                .lean()
                .session(session);
        ...
        void produceActivity({
            ...
            object: "Task",
            objectId: taskId,
        }).catch((err) => console.error(`Activity log failed for task deletion: ${taskId}`, err));
        // 7. Send the deleted task data to user
        return result;
    }
    catch (error) {
